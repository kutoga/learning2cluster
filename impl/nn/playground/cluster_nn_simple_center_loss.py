from keras.layers import Reshape, Concatenate, Bidirectional, Dense, BatchNormalization, \
    Activation, Lambda, Add, CuDNNLSTM, Dropout, add, Layer, LSTM, RepeatVector
...
from core.nn.helper import slice_layer, lukic_kl_divergence, concat, weighted_sum, reweight_values
from impl.nn.base.simple_loss.simple_loss_cluster_nn_v02 import SimpleLossClusterNN_V02

# LSTM = CuDNNLSTM

class ClusterNNSimpleCenterLoss(SimpleLossClusterNN_V02):
    ...
    def _build_network(self, network_input, network_output, additional_network_outputs):
        ...
        # Do some dropout
        # d_out = self._s_layer('embedding_dout', lambda name: Dropout(0.5, name=name))
        ...
        # self._add_debug_output(concat(embeddings_reshaped, axis=1), 'embeddings_reshaped')
        # embeddings_reshaped = reweight_values(embeddings_reshaped, self._get_clustering_hint())
        # self._add_debug_output(concat(embeddings_reshaped, axis=1), 'embeddings_processed_new')

        embeddings_reshaped_org = embeddings_reshaped
        def simple_center_loss(y_true, y_pred):
            ...
            if self.include_self_comparison:
                ...
                y_len = n * (n - 1) // 2
            y_true = Lambda(lambda x: y_true)(embeddings_reshaped_org[0])
            ...
            centers = reweight_values(embeddings_reshaped_org, y_true)
            return mean_squared_error(
                concat(centers, axis=1),
                concat(embeddings_reshaped_org, axis=1)
            )
