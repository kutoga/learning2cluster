...
class ClusterNNTry00_V21(SimpleLossClusterNN_V02):
    ...
    def _build_network(self, network_input, network_output, additional_network_outputs):
        ...
        for i in range(self.__lstm_layers):
            tmp = self._s_layer(
                'LSTM_proc_{}'.format(i), lambda name: Bidirectional(LSTM(internal_embedding_size // 2, return_sequences=True), name=name, merge_mode='sum')
            )(processed)
            tmp = self._s_layer(
                'LSTM_batch_{}'.format(i), lambda name: TimeDistributed(BatchNormalization(), name=name)
            )(tmp)
            tmp = self._s_layer(
                'LSTM_dense_{}'.format(i), lambda name: TimeDistributed(Dense(internal_embedding_size), name=name)
            )(tmp)
            tmp = self._s_layer(
                'LSTM_act_{}'.format(i), lambda name: Activation('tanh', name=name)
            )(tmp)
