from keras.layers import Reshape, Concatenate, Bidirectional, LSTM, Dense, BatchNormalization, \
    Activation, Lambda, Add, TimeDistributed
...
class ClusterNNTry00_V19(SimpleLossClusterNN_V02):
    def __init__(self, data_provider, input_count, embedding_nn=None, output_dense_units=512,
                 cluster_count_dense_layers=1, lstm_block_size=4, lstm_block_state_sizes=[128, 64, 32], output_dense_layers=1, cluster_count_dense_units=512,
        ...
        # Network parameters
        self.__lstm_block_state_sizes = lstm_block_state_sizes
        self.__lstm_block_size = lstm_block_size
        ...
        for i in range(len(self.__lstm_block_state_sizes)):
            ...
            if i > 0:
                ...
                processed = self._s_layer(
                    'INTERNAL_STATE_CHANGE{}'.format(i),
                    lambda name: TimeDistributed(Dense(lstm_block_state_size))
                )(processed)
                processed = LeakyReLU()(processed)

            for j in range(self.__lstm_block_size):
                tmp = self._s_layer(
                    'LSTM_proc_{}_{}'.format(i, j), lambda name: Bidirectional(LSTM(lstm_block_state_size // 2, return_sequences=True), name=name)
                )(processed)
