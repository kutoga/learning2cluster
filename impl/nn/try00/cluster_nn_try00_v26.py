...
class ClusterNNTry00_V26(SimpleLossClusterNN_V02):
    ...
    def _build_network(self, network_input, network_output, additional_network_outputs):
        ...
        embedding_internal_resizer_act = Activation('tanh')
        embeddings_reshaped = [embedding_internal_resizer_act(embedding) for embedding in embeddings_reshaped]

        # Use some dropout (use an own dropout for each embedding: This increases the variability)
        embeddings_reshaped = [Dropout(0.5)(embedding) for embedding in embeddings_reshaped]
