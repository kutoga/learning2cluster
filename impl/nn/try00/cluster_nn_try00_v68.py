...
class ClusterNNTry00_V68(SimpleLossClusterNN_V02):
    ...
    def _build_network(self, network_input, network_output, additional_network_outputs):
        ...
        if self._uses_embedding_layer():
            ...
            center_loss_vectors_0 = embeddings_reshaped
            def simple_center_loss_0(y_true, y_pred):
                ...
                if self.include_self_comparison:
                    ...
                    y_len = n * (n - 1) // 2
                y_true = Lambda(lambda x: y_true)(center_loss_vectors_0[0])
                ...
                centers = reweight_values(center_loss_vectors_0, y_true)
                return mean_squared_error(
                    concat(centers, axis=1),
                    concat(center_loss_vectors_0, axis=1)
                )
            self._register_additional_grouping_similarity_loss(
                'simple_center_loss_0', simple_center_loss_0, False
            )

        # Implement the "soft"-hints: Do it after the simplified center-loss
        embeddings_reshaped = reweight_values(embeddings_reshaped, self._get_clustering_hint())
        ...
        # Implement the simplified center loss
        center_loss_vectors_1 = embeddings_processed
        def simple_center_loss_1(y_true, y_pred):
            ...
            if self.include_self_comparison:
                ...
                y_len = n * (n - 1) // 2
            y_true = Lambda(lambda x: y_true)(center_loss_vectors_1[0])
            y_true = Reshape((y_len,))(y_true)
            ...
            # Calculate the centers for each vector and then the loss (just MSE)
            centers = reweight_values(center_loss_vectors_1, y_true)
            return mean_squared_error(
                concat(centers, axis=1),
                concat(center_loss_vectors_1, axis=1)
            )
        self._register_additional_grouping_similarity_loss(
            'simple_center_loss_1', simple_center_loss_1, False
        )
