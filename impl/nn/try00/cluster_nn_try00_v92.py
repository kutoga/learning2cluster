from keras.layers import Reshape, Concatenate, Bidirectional, Dense, BatchNormalization, \
    Activation, Lambda, Add, CuDNNLSTM
...
class ClusterNNTry00_V92(SimpleLossClusterNN_V02):
    def __init__(self, data_provider, input_count, embedding_nn=None, output_dense_units=512, bdlstm_block_size=4,
                 bdlstm_blocks=3, bdlstm_layers_pre_blocks=1, bdlstm_layers_post_blocks=1,
                 cluster_count_dense_layers=1, output_dense_layers=1, cluster_count_dense_units=512,
        ...
        # Network parameters
        self.__bdlstm_block_size = bdlstm_block_size
        self.__bdlstm_blocks = bdlstm_blocks
        self.__bdlstm_layers_pre_blocks = bdlstm_layers_pre_blocks
        self.__bdlstm_layers_post_blocks = bdlstm_layers_post_blocks
    ...
    def _build_network(self, network_input, network_output, additional_network_outputs):
        ...
        # Pre blocks
        for i in range(self.__bdlstm_layers_pre_blocks):
            second_last_processed = processed
            tmp = self._s_layer(
                'LSTM_pre_proc_{}'.format(i),
                lambda name: Bidirectional(LSTM(internal_embedding_size // 2, return_sequences=True), name=name)
            )(processed)
            processed = Add()([processed, tmp])

        # The "main" blocks
        for i in range(self.__bdlstm_blocks):
            for bi in range(self.__bdlstm_block_size):
                second_last_processed = processed
                tmp = self._s_layer(
                    'LSTM_block_element_{}'.format(bi), lambda name: Bidirectional(LSTM(internal_embedding_size // 2, return_sequences=True), name=name)
                )(processed)
                processed = Add()([processed, tmp])

        # Post blocks
        for i in range(self.__bdlstm_layers_post_blocks):
            second_last_processed = processed
            tmp = self._s_layer(
                'LSTM_post_proc_{}'.format(i),
                lambda name: Bidirectional(LSTM(internal_embedding_size // 2, return_sequences=True), name=name)
            )(processed)
            processed = Add()([processed, tmp])
        ...
        self._register_additional_grouping_similarity_loss(
            'simple_center_loss', simple_center_loss, False
        )
